...
function Historico() {
  ...
  const { data: partidas } = useQuery({
    ...
    staleTime: 0,
    gcTime: 0,
    queryFn: async () => (await supabase.from("partidas").select("*").eq("user_id", user!.id).order("created_at", { ascending: false }).limit(3)).data ?? [],
  });
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-6 space-y-4">
      <h1 className="font-display text-3xl uppercase italic tracking-tight">Histórico</h1>
      <p className="text-xs text-muted-foreground -mt-2">Últimas 21 partidas (até 3 campeonatos), com placar, fase e lance a lance.</p>
